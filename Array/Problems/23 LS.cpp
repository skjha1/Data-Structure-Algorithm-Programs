...
int LinearSearch(struct Array *arr, int key)
{
	...
	for (i = 0; i < arr->length; i++)
		if (key==arr->A[i]) {
			// this is known as move to head 
			swap(&arr->A[i], &arr->A[i + 1]); // swap we are using because if next time we will search that elements again then it will be easy to search that again that is we will 
			//get that in O(1)
			return i;
